...
// Stamp file recording which service release currently sits in SERVICE_DIR, so a
// crate bump re-downloads instead of silently keeping stale binaries.
const SERVICE_VERSION_STAMP = path.join(SERVICE_DIR, ".service-version");
...
function getPinnedServiceVersion() {
  ...
  try {
    ...
    log_error(`Unable to read ${lockPath}:`, err.message);
    process.exit(1);
  }
...
async function resolveServiceBundle() {
  ...
  const installedVersion = fs.existsSync(SERVICE_VERSION_STAMP)
    ? fs.readFileSync(SERVICE_VERSION_STAMP, "utf-8").trim()
    : null;
  if (
    !FORCE &&
    installedVersion === SERVICE_VERSION &&
    files.every(({ targetPath }) => fs.existsSync(targetPath))
  ) {
    log_success(
      `"clash-verge-service-ipc" ${SERVICE_VERSION} already exists, skipping download`,
    );
    return;
  }
  if (installedVersion && installedVersion !== SERVICE_VERSION) {
    log_info(
      `service binaries on disk are ${installedVersion}, pinned is ${SERVICE_VERSION} — re-downloading`,
    );
  }
  ...
  try {
    ...
    await fsp.writeFile(SERVICE_VERSION_STAMP, `${SERVICE_VERSION}\n`);
