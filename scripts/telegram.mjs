...
// Fork: send to TELEGRAM_CHAT_ID (a CI secret) so the destination chat/user id
// isn't hardcoded in this public repo. Falls back to the upstream channels if unset.
const CHAT_ID_RELEASE = process.env.TELEGRAM_CHAT_ID || "@clash_verge_re"; // 正式发布
const CHAT_ID_TEST = process.env.TELEGRAM_CHAT_ID || "@vergetest"; // 测试
...
async function sendTelegramNotification() {
  ...
  const downloadUrl =
    ...
    `https://github.com/solywsh/clash-verge-rev-fluent/releases/download/v${version}`;
  ...
  const content = `<b>🎉 <a href="https://github.com/solywsh/clash-verge-rev-fluent/releases/tag/${releaseTag}">Clash Verge Rev v${version}</a> ${releaseTitle}</b>\n\n${formattedContent}`;
    ...
    await axios.post(
      `https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`,
      {
        ...
        link_preview_options: {
          is_disabled: false,
          url: `https://github.com/solywsh/clash-verge-rev-fluent/releases/tag/v${encodedVersion}`,
